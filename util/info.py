...
def Info(token):
    ...
    [{Fore.RED}ID do usuário{Fore.RESET}]   {userID}
    [{Fore.RED}Criado em{Fore.RESET}]       {creation_date}
    [{Fore.RED}Língua{Fore.RESET}]          {language}
    ...
    {Fore.RESET}{Fore.GREEN}<───────────Informação de segurança───────────>{Fore.RESET}
    [{Fore.RED}2 Fator{Fore.RESET}]         {mfa}
    [{Fore.RED}Email{Fore.RESET}]           {email}
    [{Fore.RED}telefone{Fore.RESET}]        {phone if phone else ""}
    {Fore.RESET}{Fore.GREEN}<────────────Informações Nitro─────────────>{Fore.RESET}
    [{Fore.RED}Status Nitro{Fore.RESET}]    {has_nitro}
    [{Fore.RED}Expira em{Fore.RESET}]       {days_left if has_nitro else "0"} dias
                ''')
    if len(billing_info) > 0:
        print(f"\t\t{Fore.RESET}{Fore.GREEN}<────────────Informação de pagamento────────────>{Fore.RESET}")
        if len(billing_info) == 1:
            ...
            for i, x in enumerate(billing_info):
                title = f'Forma de pagamento #{i + 1} ({x["Payment Type"]})'
                print('' + title)
                print('' + ('=' * len(title)))
